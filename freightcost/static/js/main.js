var MSP = (function (global, undefined) {
    ...
    var checkBoxes = global.document.querySelector('#skin_changes');
	...
	moveBoxLabel.textContent = "允许运动";
	moveBoxLabel.setAttribute('for', moveBox.id);
	checkBoxes.appendChild(moveBox);
	checkBoxes.appendChild(moveBoxLabel);
	...
	yFreezeBoxLabel.textContent = "固定角度";
	yFreezeBoxLabel.setAttribute('for', yFreezeBox.id);
	checkBoxes.appendChild(yFreezeBox);
	checkBoxes.appendChild(yFreezeBoxLabel);
	...
	capeBoxLabel.textContent = "显示披风";
	capeBoxLabel.setAttribute('for', capeBox.id);
	checkBoxes.appendChild(capeBox);
	checkBoxes.appendChild(capeBoxLabel);
	...
	runBoxLabel.textContent = "模仿跑步(先勾选允许运动)";
	runBoxLabel.setAttribute('for', runBox.id);
	checkBoxes.appendChild(runBox);
	checkBoxes.appendChild(runBoxLabel);
	...
	checkBoxes.appendChild(skinBig);
	...
	skin.onload = function () {
		...
		skinc.clearRect(0, 0, 64, 64);
		
		skinc.drawImage(skin, 0, 0);
		
		var imgdata = skinc.getImageData(0, 0, 64, 64);
		...
		for(var i=0; i < 64; i+=1) {
			for(var j=0; j < 64; j+=1) {
				...
				if(isOnecolor && ((i >= 64 && i < 64 && j >= 8 && j < 16) || (i >= 40 && i < 56 && j >= 0 && j < 8))) {
					pixels[(i+j*64)*4+3] = 0;
				}
	...
	//var base64 = getBase64Image("http://localhost:8000/static/media/img/minion.png");
	//console.log(base64);
	//skin.src = 'http://localhost:8000/static/media/img/minion.png';
	
	var cacheImage = new Image();

	cacheImage.setAttribute('crossOrigin', 'anonymous');

	cacheImage.onload = function(e){
		var base64 = getBase64Image(cacheImage);
        console.log(base64);
	};
    ...
    // var showSkin = function (url) {
    //     skin.src = getBase64Image(url);
    // }

	cacheImage.src=skin_url;
	skin.src = cacheImage.src;
		...
		if(files.length > 0) {
			...
			if(file.type === 'image/png') {
				...
				fr.onload = function (e) {
					var img = new Image();
					img.onload = function () {
						if(this.width === 64 && this.height === 32 || this.width === 64 && this.height === 64) {
							skin.src = img.src;
							console.log(img.src);
						}
						else {
							alert("Error: The image must be 64×32 or 64×64 pixels!");
						}
					};
